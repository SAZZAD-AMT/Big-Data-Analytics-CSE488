...
public class PairCount {
    ...
    public static class PairCountMapper extends Mapper<LongWritable,
...
throws IOException, InterruptedException {
            ...
            for (int i = 0; i < items.length - 1; i++) {
                for (int j = i + 1; j < items.length; j++) {
                        ...
                        if(items[i].charAt(0)<items[j].charAt(0)){

                            p.set(items[i] + "," + items[j]);
                            context.write(p, o);
                        }
                        else{
                            p.set(items[j] + "," + items[i]);
                            context.write(p, o);
                        }       
                }
            }
    ...
    public static class PairCountReducer extends Reducer<Text,
